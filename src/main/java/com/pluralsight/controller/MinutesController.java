...
public class MinutesController {
	...
	public String addMinutes(@Valid @ModelAttribute ("exercise") Exercise exercise, HttpSession session, BindingResult result) {
		...
		if(result.hasErrors()) {
			...
			Goal goal = (Goal) session.getAttribute("goal");	//To obtain goal attribute from Goal controller
			exercise.setGoal(goal);								// session to assign to exercise as Foreign Key
